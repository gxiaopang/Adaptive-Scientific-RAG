"""Evaluate Hybrid RRF plus online Qwen reranking on SciFact validation."""
...
from adaptive_rag.retrieval.reranker import RerankerError, RerankingRetriever, load_reranker_config
...
DEFAULT_HYBRID_CONFIG = Path("configs/hybrid.yaml")
DEFAULT_DENSE_CONFIG = Path("configs/dense.yaml")
DEFAULT_RERANKER_CONFIG = Path("configs/reranker.yaml")
DEFAULT_BM25_INDEX_DIR = Path(".artifacts/indexes/bm25/scifact-v1")
DEFAULT_OUTPUT = Path(".artifacts/evaluations/hybrid_reranker_validation.json")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if bm25.manifest.corpus_checksum != corpus_checksum:
            raise RerankerError("BM25 index corpus checksum does not match the dataset")
        ...
        if bm25.manifest.doc_ids_checksum != snapshot.doc_ids_checksum:
            raise RerankerError("BM25 and dense indexes contain different documents")
    ...
    payload = {
        ...
        "retrieval_config": {
            "hybrid": hybrid_config.model_dump(mode="json"),
            "reranker": reranker_config.model_dump(mode="json"),
        },
        "source_manifests": {
